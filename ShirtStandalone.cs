...
//This is an enum. While I don't exactly understand how it works, I understand how to use it. It's kind of like a weird array (An array is basically a list of objects if you're not a programming nerd).
//Basically it keeps track of whatever rarity selection the user picked in the EnumPopup below. 
 ...
 * */
public class ShirtStandalone : EditorWindow
{
	...
	public RARENESS shirtRarity;
	public RenderTexture ShirtScenePreview;
	string itemName = "Red T-Shirt";
	string itemDescription = "A bright red shirt.";
	//This path is private to maintain integrity, and static to be defined as a type and not an object.
	private static string path, customNameString = ""; //url = "https://cdn.discordapp.com/attachments/296241012483817473/411961984620167168/PKGs/Shirt.unitypackage";
	...
	static void Init()
	{
		...
		window.Show();
		isOpen = true;
		/*if (!(AssetDatabase.IsValidFolder ("Shirt_Assets"))) {
			WebClient client = new WebClient();
			client.DownloadFile (url, Application.persistentDataPath + "/" + "PKGs/Shirt.unitypackage");
		}*/
	}
	void OnEnable()
	{
		//ShirtEm = new Texture2D (256, 256);
		...
		if (AssetDatabase.IsValidFolder ("Assets/Shirt_Assets")) {
		} else {
		...
		GUIContent stitleContent = new GUIContent ("Shirt Creator", icon);
		ShirtStandalone window = (ShirtStandalone)EditorWindow.GetWindow(typeof(ShirtStandalone));
	...
	void OnInspectorUpdate()
	{
		...
		if (AssetDatabase.IsValidFolder ("Assets/Shirt_Assets")) {
			...
			ShirtPreview = AssetPreview.GetAssetPreview (ShirtTex);

		} else {
			//If the mod data folder is missing/damaged, this will display the null icon for the ShirtPreview instead rather than displaying nothing.
			ShirtPreview = AssetDatabase.LoadAssetAtPath<Texture2D> ("Assets/ClothingCreator/Icons/Null.png");
		 ...
		 * 
		if (AssetDatabase.IsValidFolder ("Assets/Shirt_Assets")) {
			UnityEngine.Object prefab = AssetDatabase.LoadAssetAtPath ("Assets/Shirt_Assets/Resources/Bundles/Items/Shirts/Hoodie_Orange/Item.prefab", typeof(GameObject));
			...
			if (ShirtEm != null) {
				meshrend.material.SetTexture ("_EmissionMap", ShirtEm);
			}
			...
			if (ShirtScenePreview.Equals (null)) {
				ShirtScenePreview = new RenderTexture (210, 128, 16, RenderTextureFormat.ARGB32);
				//Without this, the texture will never be uploaded to the GPU. It's literally essential or else your texture won't work.
				ShirtScenePreview.Create ();
			}
			//Registering the actual camera component of the Main Camera duplicate we made earlier.
			if (camera.Equals (null)) {
				camera = cam.GetComponent<Camera> ();
				camera.targetTexture = ShirtScenePreview;
			}
			//Creating a new GameObject called Holder that will serve as the point of rotation for the Camera, so it can rotate around the shirt properly.
	...
	void OnInspectorUpdate()
	{
		...
		if (AssetDatabase.IsValidFolder ("Assets/Shirt_Assets")) {
			mat = AssetDatabase.LoadAssetAtPath<Material> ("Assets/Shirt_Assets/Resources/Bundles/Items/Shirts/Hoodie_Orange/Material.mat");
			ShirtTex = AssetDatabase.LoadAssetAtPath<Texture2D> ("Assets/Shirt_Assets/Resources/Bundles/Items/Shirts/Hoodie_Orange/Shirt.png");
			ShirtEm = AssetDatabase.LoadAssetAtPath<Texture2D> ("Assets/Shirt_Assets/Resources/Bundles/Items/Shirts/Hoodie_Orange/Emission.png");
		...
		else if (!AssetDatabase.IsValidFolder ("Assets/Shirt_Assets")){
			ShirtPreview = AssetDatabase.LoadAssetAtPath<Texture2D> ("Assets/ClothingCreator/Icons/Null.png");
		}
		if (AssetDatabase.IsValidFolder ("Assets/Shirt_Assets") && hasRan == false) 
		{
			mat.SetColor ("_EmissionColor", Color.black);
			mat.SetTexture ("_MetallicGlossMap", null);
			hasRan = true;
	...
	private void bundleAssets()
	{
		...
		if(path.Length > 0)
		{
			...
			//if(!BuildPipeline.BuildAssetBundle(AssetDatabase.LoadMainAssetAtPath("Assets/Shirt_Assets",typeof(UnityEngine.Object)), AssetDatabase.LoadAllAssetsAtPath("Assets/Shirt_Assets"), path, BuildAssetBundleOptions.UncompressedAssetBundle, BuildTarget.StandaloneWindows));
	...
	void OnDestroy()
	{
		GameObject pants = GameObject.Find ("Shirt");
